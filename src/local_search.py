...
def optimize_alternating_wrapper(X, W_init, H_init, LW, UW, LH, UH, max_iters=10, effort=1, time_limit=3600.0):
    """
...
def optimizeHforW(X, W, H_init, LW, UW, LH, UH, use_vnd=True):
    """
    ...
    if USE_CPP:
        X_d = X.astype(float)
        W_d = W.astype(float)
        H_opt, f = optimize_h(X_d, W_d, int(LW), int(UW), int(LH), int(UH), use_vnd)
        return H_opt, f
    else:
